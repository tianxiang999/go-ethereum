...
echo "geth --datadir $dir/test-genesis init \
        $GOPATH/src/github.com/ethereum/go-ethereum/genesis-test.json "
geth --datadir $dir/test-genesis init \
        $GOPATH/src/github.com/ethereum/go-ethereum/genesis-test.json 
...
if [ ! -f "$dir/nodes" ]; then
    ...
    for ((i=0;i<$num;++i)); do
        ...
        echo "create node $id"
        geth --identity "MyTestNode1" --nodiscover --networkid $networkid \
        ...
        eth="geth --identity "MyTestNode$id" --nodiscover --networkid $networkid \
